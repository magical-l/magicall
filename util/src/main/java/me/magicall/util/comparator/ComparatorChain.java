...
public class ComparatorChain<T> implements Comparator<T>, Serializable {
	 ...
	 */
	public ComparatorChain(final Comparator<? super T> comparator, final boolean reverse) {
		chain = new ArrayList<>();
	...
	public ComparatorChain(final Comparator<? super T>... comparator) {
		if (comparator.length != 0) {
			...
			chain = new ArrayList<>(Arrays.asList(comparator));
	 ...
	 */
	public ComparatorChain(final List<Comparator<? super T>> list) {
	 ...
	 */
	public ComparatorChain(final List<Comparator<? super T>> list, final BitSet bits) {
		chain = list;
		orderingBits = bits;
	}
	 ...
	 */
	public ComparatorChain<T> addComparator(final Comparator<? super T> comparator) {
		addComparator(comparator, false);
		return this;
	}
	 ...
	 */
	public ComparatorChain<T> addComparator(final Comparator<? super T> comparator, final boolean reverse) {
		checkLocked();
	 ...
	 * size()
	 */
	public ComparatorChain<T> setComparator(final int index, final Comparator<? super T> comparator) throws IndexOutOfBoundsException {
	 ...
	 * @param reverse false = forward sort order; true = reverse sort order
	 */
	public ComparatorChain<T> setComparator(final int index, final Comparator<? super T> comparator, final boolean reverse) {
		checkLocked();
	 ...
	 */
	public ComparatorChain<T> setForwardSort(final int index) {
		checkLocked();
		orderingBits.clear(index);
		return this;
	}
	 ...
	 * @param index Index of the ComparatorChain
	 */
	public ComparatorChain<T> setReverseSort(final int index) {
		checkLocked();
		orderingBits.set(index);
		return this;
	}
	 ...
	 */
	@Override
	public int compare(final T o1, final T o2) throws UnsupportedOperationException {
		if (!isLocked) {
			checkChainIntegrity();
			isLocked = true;
		}
